...
- Keep superseded tool environments available while Volta-launched processes use them, so reinstalling or upgrading does not break running CLIs such as Codex. A later successful update collects environments after those processes exit. Installations created before execution tracking remain until `volta tool uninstall`.
